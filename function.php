...
function base_url() //This is base Url string.
{
	return 'http://localhost/LMS/';
}

function is_admin_login() //Check Admin is logged in or not.
{
...
function is_user_login() //Check User is logged in or not.
{
...
function set_timezone($connect) //This will set our Web App according to the library Time-Zone
{
	$query = "
...
function get_date_time($connect) //This will return current time and date.
{
...
function get_one_day_fines($connect) //Automatically enter fine.
{
	$output = 0;
	$query = "
...
function get_currency_symbol($connect) //Return the currency symbol. (currency we selected already)
{
	$output = '';
	$query = "
...
function get_book_issue_limit_per_user($connect) //It will return total book limit per user.
{
	$output = '';
	$query = "
...
function get_total_book_issue_per_user($connect, $user_unique_id) //It will return total issued book by user.
{
	$output = 0;
	$query = "
...
function get_total_book_issue_day($connect) //It will return total book limit per day.
{
	$output = 0;
...
function currency_array() //All the currencies are store here.
	{
		$currencies = array(
			array('code'=> 'ALL',
			'countryname'=> 'Albania',
	...
	function Currency_list() //When user click on currency box it will show all the curreny list
	{
		$html = '
	...
	function Book_list($connect) { //When user click on Book box it will show all the Book list
		$html = '
			<option value="">Select Book</option>
		';

		$bookListQuery = 
		"SELECT book_isbn_number, book_name FROM lms_book WHERE book_status = 'Enable'";
		$data = $connect->query($bookListQuery);

		foreach($data as $row) {
			$html .= '<option value="'.$row["book_isbn_number"].'">'.$row["book_name"].'</option>';
		}
		
		return $html;
	}

	function User_list($connect) { //When user click on User box it will show all the User list
		$html = '
			<option value="">Select User</option>
		';

		$userListQuery = 
		"SELECT user_unique_id, user_name FROM lms_user WHERE user_status = 'Enable'";
		$data = $connect->query($userListQuery);

		foreach($data as $row) {
			$html .= '<option value="'.$row["user_unique_id"].'">'.$row["user_name"].'</option>';
		}
	...
	function Timezone_list() //All the time zones are saved here.
	{
		$timezones = array(
		...
		//When user click on time-zone box it will show all the time-zone list
...
function Count_total_issue_book_number($connect) //Check number of books issued to user in Database.
{
	$total = 0;
...
function Count_total_returned_book_number($connect) //Check number of books return to library in Database.
{
	$total = 0;
...
function Count_total_not_returned_book_number($connect)  //Check number of books Not return to library in Database.
{
	$total = 0;
...
function Count_total_fines_received($connect)  //Check Fine on books which are returned late to library in Database.
{
	$total = 0;
...
function Count_total_book_number($connect) //Check number of books in Database.
{
	$total = 0;
