...
?>
<!-- This will display all issued books, return books & fines. -->
			...
			<table id="datatablesSimple">
				...
				if($statement->rowCount() > 0)
				{
					foreach($statement->fetchAll() as $row)
					{
						...
						if($status == 'Issue') //check book is issued or not.
						{
							$status = '<span class="badge bg-warning">Issue</span>';
						}

						if($status == 'Not Return') //check book is not returned.
						{
							$status = '<span class="badge bg-danger">Not Return</span>';
						}

						if($status == 'Return') //check book is returned.
						{
							$status = '<span class="badge bg-primary">Return</span>';
						}

						//Show complete issued book data.
						echo '
						<tr>
