...
{
	if($_POST["action"] == 'search_book_isbn') //Search algorithm for book.
	{
		$query = "
		...
		foreach($result as $row) //All find books
		{
			$data[] = array(
	...
	if($_POST["action"] == 'search_user_id') //Search algorithm for user.
	{
		$query = "
		...
		$result = $connect->query($query);  //All find Users
		$data = array();
